...
void mostrarDatosPantalla() {
    static int pagina = 0; // Variable para rastrear la página actual
    ...
    switch (pagina) {
        ...
        case 1:
            tft.setCursor(0, 0);
            tft.print("Hum Suelo Cap: ");
            tft.println(humedadSueloCap);
            tft.print("Hum Suelo Res: ");
            tft.println(humedadSueloRes);
            tft.print("Distancia: ");
            tft.println(distancia);
            tft.print("Iluminacion: ");
            tft.println(Iluminacion ? "ON" : "OFF");
            break;

        case 2:
            tft.setCursor(0, 0);
            tft.print("Estado Bomba: ");
            tft.println(Bomba ? "ACTIVADA" : "DESACTIVADA");
            tft.print("Nodo ID: ");
            tft.println(nodoID_LoRa);
            break;
    }

    // Cambiar a la siguiente página después de un intervalo de tiempo
    delay(2000); // Pausa de 2 segundos para visualizar cada página
    pagina = (pagina + 1) % 3; // Cambiar a la siguiente página cíclicamente
}
